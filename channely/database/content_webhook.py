...
from channely.database.enums import ContentEventType
...
class ContentWebhookEntity(BaseEntityWithoutTimestamps):
    ...
    # Relationships
    headers: Mapped[list["ContentWebhookHeaderEntity"]] = relationship("ContentWebhookHeaderEntity", back_populates="webhook")


class ContentWebhookHeaderEntity(BaseEntity):
    __tablename__ = "content_webhook_headers"
    
    content_webhook_id: Mapped[UUID] = mapped_column(PostgresUUID(as_uuid=True), ForeignKey("content_webhooks.id"), nullable=False)
    key: Mapped[str] = mapped_column(String(255), nullable=False)
    value: Mapped[str] = mapped_column(String(1024), nullable=False)
    
    # Relationships
    webhook: Mapped["ContentWebhookEntity"] = relationship("ContentWebhookEntity", back_populates="headers")